...
export default function LoginPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-black dark:bg-zinc-950 px-4 py-12">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-blue-900/20 via-black to-black"></div>
      ...
      <div className="z-10 w-full max-w-sm p-8 backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl shadow-2xl">
        ...
        <form action={action} className="space-y-6">
          ...
          >
            {isPending ? t("loading_login") : t("submit")}
          </button>
        </form>

        <p className="mt-8 text-center text-sm text-zinc-500">
          {t("noAccount")}{" "}
          <Link href="/register" className="text-white hover:underline decoration-white/30 underline-offset-4">
            {t("registerLink")}
          </Link>
        </p>
      </div>
